...
-- retreiving all rows and columns of all tables
...
-- retriving rows with specific id
...
-- get the number of appoinments by specialty
...
-- find the most common reason for appoinments
select reason, count(*) as count from appointments 
group by reason 
order by count desc;

-- list patients with their latest appoinment date
...
-- find prescription pending
select prescription_id, pr.medication, pr.dosage, pr.instructions
from prescriptions pr
join appointments a on pr.appointment_id = a.appointment_id
join billing b on a.appointment_id = b.appointment_id
where b.status = 'pending';

-- detailed view of each patient, including their appointment and billing stauts
...
-- number of appoinment as per months
